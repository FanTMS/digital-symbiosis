...
type ServiceCategory = 'education' | 'it' | 'design' | 'languages' | 'business' | 'lifestyle' | 'writing' | 'music' | 'other';
...
const CreateServicePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (title.trim().length < 5) newErrors.title = 'Название должно быть не короче 5 символов';
    if (title.trim().length > 100) newErrors.title = 'Название слишком длинное';
    if (description.trim().length < 20) newErrors.description = 'Описание должно быть не короче 20 символов';
    if (description.trim().length > 1000) newErrors.description = 'Описание слишком длинное';
    if (!category) newErrors.category = 'Выберите категорию';
    const priceNum = Number(price);
    if (!price || isNaN(priceNum) || priceNum < 1 || priceNum > 10000) newErrors.price = 'Цена должна быть от 1 до 10 000 кредитов';
    if (skills.length === 0) newErrors.skills = 'Добавьте хотя бы один навык';
    if (skills.some(s => s.length < 2 || s.length > 30)) newErrors.skills = 'Навыки должны быть от 2 до 30 символов';
    setErrors(newErrors);
    ...
    <div className="pb-20 pt-2 min-h-screen bg-white overflow-auto">
      <div className="px-4">
        ...
        >
          {/* Title */}
          <div className="mb-4">
            <label htmlFor="title" className="block font-medium mb-1">Название услуги</label>
            <input
              type="text"
              id="title"
          ...
          {/* Description */}
          <div className="mb-4">
            <label htmlFor="description" className="block font-medium mb-1">Описание услуги</label>
            <textarea
              id="description"
          ...
          {/* Price */}
          <div className="mb-4">
            <label htmlFor="price" className="block font-medium mb-1">Стоимость (в кредитах)</label>
            <input
              type="number"
              id="price"
              min="1"
