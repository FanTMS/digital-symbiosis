...
const ReferralsAdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from('referrals')
        .select('*, referrer:users!referrals_referrer_id_fkey(*), referred:users!referrals_referred_id_fkey(*)')
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
      <h2 className="text-xl font-bold mb-4">Отчёт по рефералам и промокодам</h2>
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} className="border-b">
                  ...
                  <td className="p-2">{r.bonus_received ? '20/10 кр.' : '-'}</td>
