...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, onFavoriteChange }) => {
  ...
  const handleFavorite = async (e: React.MouseEvent) => {
    ...
    setLoading(true);
    if (isFavorite) {
      await supabase.from('favorites').delete().eq('user_id', currentUser.id).eq('service_id', service.id);
      setIsFavorite(false);
    } else {
      await supabase.from('favorites').insert({ user_id: currentUser.id, service_id: service.id });
      setIsFavorite(true);
    }
    setLoading(false);
