...
const ProfileCard: React.FC<ProfileCardProps> = ({ user, detailed = false }) => {
      ...
      <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-4">
        <div className="flex items-center">
          <div className="relative">
            <img
              src={user.avatarUrl || 'https://images.pexels.com/photos/4926674/pexels-photo-4926674.jpeg?auto=compress&cs=tinysrgb&w=150'}
              alt={user.name}
              className="w-16 h-16 rounded-full object-cover border-2 border-white"
            />
            <div className={`absolute -bottom-1 -right-1 rounded-full p-1 bg-white`}>
              <span className={`flex items-center justify-center w-5 h-5 ${getLevelColor(user.level)} text-xs font-bold rounded-full`}>
                {user.level === 'Новичок' ? 'Н' : user.level === 'Специалист' ? 'С' : user.level === 'Эксперт' ? 'Э' : 'М'}
              </span>
            </div>
          </div>
