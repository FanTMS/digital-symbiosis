...
export const challengesApi = {
    ...
    // Получить количество голосов за работу
    async getVotes(submissionId: string) {
        const { data, error } = await supabase
    ...
    async hasVoted(submissionId: string, userId: number) {
        const { data, error } = await supabase
            .from('challenge_votes')
            .select('id')
            .eq('submission_id', submissionId)
    ...
    async vote(submissionId: string, userId: number) {
        const { data, error } = await supabase
            .from('challenge_votes')
