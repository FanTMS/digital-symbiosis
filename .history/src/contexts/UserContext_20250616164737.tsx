...
import { uploadTelegramAvatar } from "../lib/uploadTelegramAvatar";
        ...
        try {
          ...
          if (userError && userError.code === "PGRST116") {
            ...
            if (telegramUser.photo_url) {
              try {
                avatarUrl = await uploadTelegramAvatar(telegramUser.photo_url, telegramUser.id);
              } catch (e) {
                logErrorToTelegram(e, "UserContext: uploadTelegramAvatar");
              }
            }
          ...
          // Если у пользователя есть Telegram photo_url, а avatar_url отсутствует или это ссылка на Telegram CDN
          if (telegramUser.photo_url && (!data.avatar_url || data.avatar_url.includes('t.me/i/userpic') || data.avatar_url.includes('telegram-cdn.org'))) {
            try {
              const uploadedUrl = await uploadTelegramAvatar(telegramUser.photo_url, telegramUser.id);
              if (uploadedUrl && data.avatar_url !== uploadedUrl) {
                await supabase.from('users').update({ avatar_url: uploadedUrl }).eq('id', telegramUser.id);
                data.avatar_url = uploadedUrl;
              }
            } catch (e) {
              logErrorToTelegram(e, "UserContext: uploadTelegramAvatar update");
            }
          }
