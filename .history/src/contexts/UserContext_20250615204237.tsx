      ...
      const fetchUser = async () => {
        try {
          ...
          if (telegramUser.photo_url && (!data.avatar_url || data.avatar_url.includes('t.me/i/userpic'))) {
            // Просто используем ссылку Telegram
            if (data.avatar_url !== telegramUser.photo_url) {
              await supabase.from('users').update({ avatar_url: telegramUser.photo_url }).eq('id', telegramUser.id);
              data.avatar_url = telegramUser.photo_url;
            }
          }
