...
export default function QuizEditor({ quiz, questions: initialQuestions, onSave }: QuizEditorProps) {
    ...
    return (
        <div className="max-w-2xl mx-auto p-4 bg-white rounded-xl shadow">
            <h2 className="text-xl font-bold mb-4">{quiz ? 'Редактировать квиз' : 'Создать квиз'}</h2>
            <div className="mb-4">
                    ...
                    <div key={idx} className="bg-blue-50 rounded-lg p-4 shadow-sm">
                        ...
                        {(q.type === 'single' || q.type === 'multiple') && (
                            <div>
                                ...
                                <Button size="xs" variant="outline" onClick={() => addOption(idx)}>Добавить вариант</Button>
                            </div>
                        )}
                    </div>
                ))}
            </div>
            <Button variant="outline" className="mb-6" onClick={addQuestion}>Добавить вопрос</Button>
