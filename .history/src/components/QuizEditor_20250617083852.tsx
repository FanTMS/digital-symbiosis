...
export default function QuizEditor({ quiz, questions: initialQuestions, onSave }: QuizEditorProps) {
    ...
    return (
        <div className="max-w-xl w-full mx-auto p-2 sm:p-6 bg-white rounded-xl shadow min-h-[80vh] flex flex-col relative" style={{ paddingBottom: '110px', minHeight: '80vh' }}>
                ...
                {questions.map((q, idx) => (
                    <div key={idx} className="bg-blue-50 rounded-xl p-4 shadow-sm flex flex-col gap-2">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <Button size="xs" variant="ghost" onClick={() => removeQuestion(idx)} disabled={questions.length === 1}>Удалить</Button>
                        </div>
                        <input
                                    ...
                                    {(q.options || []).map((opt, oIdx) => (
                                        <div key={oIdx} className="flex gap-2 mb-1">
                                            <input
                                                ...
                                                maxLength={100}
                                            />
                                            <Button size="xs" variant="ghost" onClick={() => removeOption(idx, oIdx)} disabled={(q.options?.length || 0) <= 1}>-</Button>
                                        </div>
                                    ))}
                                    <Button size="xs" variant="outline" onClick={() => addOption(idx)} className="w-full sm:w-auto">Добавить вариант</Button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <Button variant="outline" className="w-full py-2 rounded-xl mb-4" onClick={addQuestion}>Добавить вопрос</Button>
            ...
            <div className="fixed left-0 right-0 bottom-0 z-40 px-2 pb-3 animate-fade-in">
                <Button variant="primary" onClick={handleSave} loading={saving} disabled={saving} fullWidth className="py-4 text-base rounded-2xl shadow-xl font-bold">
                    Сохранить квиз
                </Button>
            </div>
        </div>
