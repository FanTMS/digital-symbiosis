...
export default function QuizEditor({ quiz, questions: initialQuestions, onSave }: QuizEditorProps) {
    ...
    useEffect(() => {
        if (!tg) return;
        tg.MainButton.setText(quiz ? 'Сохранить изменения' : 'Создать квиз');
        tg.MainButton.show();
    ...
    }, [tg, quiz, title, description, questions]);

    useEffect(() => {
        if (!tg) return;
        if (saving || !title.trim() || questions.some(q => !q.question?.trim())) {
            tg.MainButton.disable();
        } else {
            tg.MainButton.enable();
        }
    }, [tg, saving, title, questions]);
    ...
    return (
        ...
        >
            ...
            {/* Кнопка сохранения только если нет tg (например, не в Telegram) */}
            {!tg && (
