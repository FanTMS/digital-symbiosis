...
const EditChallengeModal: React.FC<ChallengeFormProps> = ({ challenge, onClose, onSaved }) => {
    ...
    async function handleSave(e: React.FormEvent) {
        e.preventDefault();
        setSaving(true);
        await supabase.from('challenges').update({ title, prize }).eq('id', challenge.id);
        setSaving(false);
        onSaved();
        onClose();
    }
    ...
    return (
        <Modal isOpen onClose={onClose}>
            <form onSubmit={handleSave} className="p-4 w-[90vw] max-w-md space-y-4">
                <h3 className="text-lg font-bold">Редактировать челлендж</h3>
                <div>
                ...
                <div>
                    ...
                    <input value={prize} onChange={e => setPrize(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 bg-gray-50 focus:ring-2 focus:ring-primary-200" />
                </div>
