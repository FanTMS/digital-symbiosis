...
const HowItWorksGame: React.FC<HowItWorksGameProps> = ({ onFinish, submitting }) => {
            ...
            <AnimatePresence mode="wait">
                {isLast ? (
                    ...
                    >
                        ...
                        {onFinish ? (
                            <button
                                className="mt-2 px-6 py-3 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-xl font-bold shadow hover:from-cyan-500 hover:to-blue-600 transition text-base disabled:opacity-60"
                                onClick={onFinish}
                                disabled={submitting}
                            >
                                {submitting ? 'Загрузка...' : 'Начать пользоваться'}
                            </button>
                        ) : (
                            <button
