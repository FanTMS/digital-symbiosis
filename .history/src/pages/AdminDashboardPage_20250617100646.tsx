...
function PromoBannerAdminForm() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 500 * 1024) { // 500 КБ
        setError("Размер изображения не должен превышать 500 КБ");
        return;
      }
      setImageFile(file);
      ...
      reader.onload = (ev) => {
        ...
        if (result.length > 700000) { // ~500 КБ base64
          setError("Размер base64 изображения слишком большой (макс. 500 КБ)");
          return;
        }
        setImagePreview(result);
        localStorage.setItem('promo_banner_image', result);
        setError(null);
      };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (image_url && image_url.length > 700000) {
      setError("Размер base64 изображения слишком большой (макс. 500 КБ)");
      setSaving(false);
      return;
    }
    try {
      ...
      localStorage.removeItem('promo_banner_image');
    } catch (e: any) {
