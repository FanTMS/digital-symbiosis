...
function PromoCodeActivation({ user }: { user: any }) {
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    if (activationsError) {
      setError("Ошибка проверки лимита активаций");
      setLoading(false);
      return;
    }
    if ((activationsCount || 0) >= (promo.max_activations || 1)) {
      setError("Достигнут лимит активаций для этого промокода");
      setLoading(false);
      return;
    }
    // Проверяем, активировал ли этот пользователь уже этот промокод
    const { count: userActivations } = await supabase
      .from("promo_code_activations")
      .select("id", { count: "exact", head: true })
      .eq("promo_code_id", promo.id)
      .eq("user_id", user.id);
    if ((userActivations || 0) > 0) {
      setError("Вы уже активировали этот промокод");
      setLoading(false);
      return;
    }
    // Всё ок — начисляем кредиты и отмечаем активацию
    await supabase.from("users").update({ credits: (user.credits || 0) + promo.amount }).eq("id", user.id);
    await supabase.from("promo_code_activations").insert({ promo_code_id: promo.id, user_id: user.id, activated_at: new Date().toISOString() });
    // Если лимит исчерпан после этой активации — деактивируем промокод
    if ((activationsCount || 0) + 1 >= (promo.max_activations || 1)) {
