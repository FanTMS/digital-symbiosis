...
const ServiceDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (service?.quiz_id) {
      setQuizLoading(true);
      import('../lib/quizApi').then(({ getQuizQuestions }) =>
        getQuizQuestions(service.quiz_id as string)
          ...
          <div className="ml-auto flex items-center">
            ...
            {user && user.id !== provider.id && (
              <button
                ...
                className={`ml-3 w-10 h-10 rounded-full transition-all duration-200 flex items-center justify-center touch-manipulation select-none
                  ${isFavorite
              ...
              >
                <Heart
                  ...
                  strokeWidth={2}
                  className={`transition-all duration-200 ${favoriteLoading ? 'animate-pulse' : ''}`}
                />
              </button>
            )}
          </div>
