  ...
  const send = async () => {
    ...
    try {
      if (chatFile) {
        ...
        console.log('[DEBUG] Загружаю файл:', filePath, chatFile);
        const { error: uploadError } = await supabase.storage
          .from("chat-attachments")
          .upload(filePath, chatFile);
        if (uploadError) {
          console.error('[DEBUG] Ошибка загрузки файла:', uploadError);
          throw uploadError;
        }
        const { data: publicUrlData } = supabase.storage
          .from("chat-attachments")
          .getPublicUrl(filePath);
        console.log('[DEBUG] Получена публичная ссылка:', publicUrlData);
      ...
      const { data: message, error: msgError } = await supabase
        ...
        .single();
      if (msgError) {
        console.error('[DEBUG] Ошибка создания сообщения:', msgError);
        throw msgError;
      }
      ...
      if (attachmentUrl && messageId) {
        ...
        if (attachError) {
          console.error('[DEBUG] Ошибка добавления attachment:', attachError);
          throw attachError;
        }
        console.log('[DEBUG] Attachment успешно добавлен:', attachmentUrl);
      }
      ...
      console.error('[DEBUG] Общая ошибка:', err);
    }
    setUploading(false);
  };
