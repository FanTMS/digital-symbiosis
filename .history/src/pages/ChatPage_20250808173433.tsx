...
import { useUpdatePriceProposalStatus } from '../hooks/usePriceProposals';
...
export default function ChatPage() {
  ...
  const updateProposalStatus = useUpdatePriceProposalStatus();
    ...
    <div className="chat-redesign-root">
      ...
      <header className="chat-header">
        ...
        <div className="chat-header-avatar">
          <Avatar src={otherUser?.avatar_url} name={otherUser?.name} size={48} />
        </div>
      ...
      <main className="chat-messages-list pb-20 sm:pb-24" id="chat-messages-list">
        {messages.map((msg, idx) => {
          // Скрываем целевые системные сообщения не для текущего пользователя
          const messageRole = msg?.meta?.role as string | undefined;
          if (messageRole && msg.sender_id !== user?.id) {
            return null;
          }
          ...
          return (
            <div key={msg.id} className={`chat-message-row ${isOwn ? 'own' : 'other'}`}>
              <div className={`chat-message-bubble ${isOwn ? 'own' : 'other'}`}>
                ...
                {/* Текст сообщения */}
                {msg.meta?.type === 'price_proposal' ? (
                  <div className="chat-message-text">
                    <div className="mb-2">
                      <b>Предложена новая цена:</b> {msg.meta.proposedPrice} кр.
                    </div>
                    {user?.id === msg.meta.toUserId && msg.meta.status !== 'accepted' && msg.meta.status !== 'rejected' && (
                      <div className="flex gap-2 mt-2">
                        <Button
                          variant="success"
                          size="sm"
                          disabled={updateProposalStatus.isLoading && actionMsgId === msg.id}
                          onClick={async () => {
                            setActionMsgId(msg.id);
                            setActionError(null);
                            try {
                              await updateProposalStatus.mutateAsync({ id: msg.meta.priceProposalId, status: 'accepted' });
                              await chatApi.sendMessage(
                                chatId,
                                user.id,
                                `Вы приняли предложение по цене: ${msg.meta.proposedPrice} кр.`,
                                { type: 'price_proposal_accepted', priceProposalId: msg.meta.priceProposalId }
                              );
                            } catch (e: any) {
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            setActionError(null);
                            try {
                              await updateProposalStatus.mutateAsync({ id: msg.meta.priceProposalId, status: 'rejected' });
                              await chatApi.sendMessage(
                                chatId,
                                user.id,
                                `Вы отклонили предложение по цене: ${msg.meta.proposedPrice} кр.`,
                                { type: 'price_proposal_rejected', priceProposalId: msg.meta.priceProposalId }
                              );
                            } catch (e: any) {
                              setActionError(e.message || 'Ошибка');
                            }
                    ...
                    {(msg.meta.status === 'accepted' || msg.meta.status === 'rejected') && (
                      <div className="text-xs mt-2 text-gray-500">
                        {msg.meta.status === 'accepted' ? 'Предложение принято' : 'Предложение отклонено'}
                      </div>
                    )}
                  </div>
                ) : (
