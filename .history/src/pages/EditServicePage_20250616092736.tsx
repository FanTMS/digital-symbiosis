...
const EditServicePage: React.FC = () => {
    ...
    const handleDelete = async () => {
        if (!service) return;
        if (!window.confirm('Вы уверены, что хотите удалить эту услугу? Это действие необратимо.')) return;
        setSaving(true);
        try {
    ...
    return (
        <div className="max-w-xl mx-auto p-4 bg-white rounded-xl shadow">
            ...
            <Button variant="danger" onClick={handleDelete} disabled={saving} className="mt-4 w-full">
                Удалить услугу
            </Button>
        </div>
    );
