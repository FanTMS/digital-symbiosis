...
const ChallengeDetailPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            // TODO: Загрузка файла в Supabase Storage и получение file_url
            const file_url = file.name; // временно, заменить на реальный url после загрузки
                    ...
                    {(submissions || []).filter((s: any) => s.status === 'approved').length === 0 ? (
                        ...
                        (submissions || []).filter((s: any) => s.status === 'approved').map((s: any) => (
                            <div key={s.id} className="bg-white rounded-lg shadow p-3">
                                <div className="font-medium">{s.file_url}</div>
